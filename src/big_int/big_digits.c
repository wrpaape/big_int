			      ...
			      const size_t count)
{
	...
	case NEG:
		digit_t app_res[mlt_cnt2 + count + 1lu];
			...
			const size_t count2)
{
	...
	for (i = 1lu; i < count2; ++i) {
		...
		if (carry) {
			...
			if (small >= large) {
				digits[i] = (9u + large) - small;

			} else {
				digits[i] = large - (small + 1lu);
				carry = false;
			}
			...
			if (small > large) {
				digits[i] = (10u + large) - small;
				carry = true;

			} else {
				digits[i] = large - small;
			}
	...
	for (i = count1 - 1lu; (i > 0lu) && (digits[i] == 0u); --i);
