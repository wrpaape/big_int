				      ...
				      word_t *words)
{
	...
	HANDLE_MALLOC(result, sizeof(struct BigDigits));
	...
	HANDLE_MALLOC(base, sizeof(digit_t) * buff_alloc * 4lu);
	...
	size_t base_acc_cnt = DIGITS_PER_WORD_BASE;
	...
	while (1) {
		buff1_cnt = multiply_digits_by_word(buff1,
						    base_acc,
						    base_acc_cnt,
						    words[i]);
...
RESIZE_AND_RETURN:
	HANDLE_REALLOC(res_digits, result->)
}
