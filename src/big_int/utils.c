...
extern inline buff_t add_word_to_buffer(buff_t buffer,
					word_t word);

extern inline buff_t multiply_word_by_digit(word_t word,
					    digit_t digit);

extern inline word_t upper_word(buff_t buffer);

extern inline word_t lower_word(buff_t buffer);

extern inline enum Sign compare_big_ints(struct BigInt *big1,
					 struct BigInt *big2);
